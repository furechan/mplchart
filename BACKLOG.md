...
- Add support for pandas native expressions — a first-class pandas expression type (analogous to `pl.Expr`) so the pandas backend has parity with the polars expression path in `resolve_expr`.
