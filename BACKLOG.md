...
- Rename `ExprBundle` → `ExprTuple` in `expressions/prelude.py`. The class is a `tuple` subclass with a `.label` — `ExprTuple` names the shape more precisely than "Bundle", and frees "bundle" terminology for the companion frozen-dict input type (`ExprDict` / `PRICES`) being designed on the bearta/next-gen side. See [`~/Projects/dev-notes/expression-tuple-pattern.md`](~/Projects/dev-notes/expression-tuple-pattern.md) and [`~/Projects/dev-notes/expression-factory-signatures.md`](~/Projects/dev-notes/expression-factory-signatures.md).
