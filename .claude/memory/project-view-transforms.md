...
Conventions: windowing counts bricks/columns; renko volume is per-brick (even-split on multi-brick bars, conserved), pnf volume is a per-bar rate (mean bar volume over column lifetime) — deliberately different. Renko same-bar bricks are nudged +1ns (pandas slice goes cartesian on duplicate dates — real fix, a positional slice fast path, still open). Other open items: percentage boxes, high/low-based pnf.
