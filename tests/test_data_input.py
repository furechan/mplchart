...
from mplchart.primitives import Area, Bars, Line
...
def test_bar_and_area_accept_data(prices):
    ...
    chart = Chart(prices, figsize=(4, 3))
    chart.pane("below").plot(Bars(delta))
    chart.pane("below").plot(Area(delta))
    close_figure(chart)


def test_data_repr_is_truncated(prices):
    plot = Line(prices["close"])
...
def test_polars_series_positional():
    ...
    chart = Chart(prices, figsize=(4, 3))
    with pytest.raises(ValueError, match="full-length"):
        chart.plot(Line(pl.Series("x", [1.0, 2.0])))
    close_figure(chart)
