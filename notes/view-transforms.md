...
- Duplicate completion dates (multi-brick days) **confirmed broken on pandas** (renko-proto, 2026-07-25): `PandasDataView.slice` aligns the windowed xloc against the full frame by date, and the inner join goes cartesian on duplicate labels — rows multiply and duplicate x-values collapse the candle body width to zero via `nanmin(diff)`. Polars is positional and unaffected. Prototype workaround: nudge same-bar bricks +1ns each (invisible in labels). Candidate real fix: a positional fast path in `slice` when `data is view.prices`.
...
- ~~Promote~~ — done (2026-07-25): `primitives/renko.py` (`calc_renko` + `Renko`) and `primitives/pointfigure.py` (`calc_pnf` + `PointFigure`), both binding their transform via `chart.get_view(transform=self.transform)` (bound method); `wrap_result(dates=...)` + `get_dates` in utils; tests in `tests/test_renko.py` / `tests/test_pointfigure.py` (backend-parametrized, hand-crafted sequences). The +1ns nudge shipped as-is — the pandas duplicate-dates slice fix (positional fast path when `data is view.prices`) remains open.
