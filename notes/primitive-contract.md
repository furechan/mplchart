...
- **LinePlot, BarPlot, AreaPlot, Stripes**: `view.eval(ind)` → `view.series_xy(result)` → draw. Single-output only. An indicator is **required** (`required_indicator()` raises otherwise) — bare instances were only meaningful under the removed chaining cache.
