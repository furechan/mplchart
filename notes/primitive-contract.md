...
### 1. Compute → window (`view.eval` + `view.series_xy`)

Full-length series computed first, windowed at extraction. The default for anything that plots a computed series.

- **Line, Bars, Area, Stripes**: `view.eval(ind)` → `view.series_xy(result)` → draw. Single-output only. An indicator is **required** (`required_indicator()` raises otherwise) — bare instances were only meaningful under the removed chaining cache.
...
`view.eval` for the analytics, a `chart.view.prices` column for placement — two already-aligned full-length arrays (both in prices row order by construction), windowed together by the variadic `series_xy`. No join or alignment happens; the variadic call only guarantees both are cut by the same window slice:

- **Markers**: `view.eval(ind)` → derive the signal flag; `chart.view.prices["close"]` → y-placement; `series_xy(flag, close)` applies one window slice to both against one shared x. The only variadic `series_xy` call site — used because the two arrays come from different sources, so neither frame idiom (`slice(xcol=...)`) applies.
...
Pattern 1's shape, self-sourced: computes a price-derived series from `chart.view.prices` via its own `__call__` (`calc_price(prices, item)`), then `series_xy(series)` → draw.

- The `__call__` doubles as a plain callable indicator, so `Price("close")` also works anywhere a callable indicator is accepted (`Line(Price("open"))`, `prices.pipe(...)`).
- Kin to pattern 4 in spirit (renders the price frame) but series-shaped in mechanics.

### 4. Window prices → read columns (`slice(chart.view.prices, xcol=...)`)
...
- **Candlesticks, OHLC, Volume**: `slice(chart.view.prices, xcol="xloc")` → read `xloc` + price columns → draw.
...
- **OHLC mode** (`indicator=None`): `slice(chart.view.prices, xcol="xloc")` → peak kernel on `high` / valley kernel on `low` → sparse indices into `xloc`. Pattern 3's sequence feeding a geometry kernel; `view.eval` is never called.
- **Series mode** (bound indicator, constructor or `@`): `view.eval(ind)` → `view.series_xy(result)` → both peaks and valleys on the same values → sparse sub-selection of the windowed arrays. Pattern 1's sequence feeding a geometry kernel. The result must be a single series (explicit `ValueError` otherwise — compose a single-output expression).
...
- **HLine, Pane**: axes-only (`canvas.get_axes`); never touch data.
...
- **Analytics compute full-length, then window** (the `view.eval` of patterns 1, 2 and 4): indicators need the full history for warmup (an EMA's value inside the window depends on data before it). Windowing first would change the numbers.
...
Matplotlib-side surface, implemented by the `Canvas` — unaffected by any data-core swap.

| Member | Call sites | Contract |
|---|---|---|
| `chart.canvas.get_axes(target=None)` | 12 | Current or target pane axes. |
| `chart.canvas.get_color(name, ax, fallback=)` | 4 | Color resolution/cycling per axes: color_scheme lookup (raw name, then prefix), list cycling, `~` closest-color, `line`/`fill` dynamic fallback. The former `indicator` parameter was dead (rich-indicator era vestige) and removed 2026-07-18. |
| `chart.canvas.root_axes()` | 2 | Root (background) axes. |
| `chart.canvas.main_axes()` | 1 | Main price axes. |
...
- **`slice` has no free parameters in practice.** All 5 runtime calls are character-for-character `chart.slice(chart.view.prices, xcol="xloc")` — `data` is always prices, `xcol` always `"xloc"`. A no-argument windowed-prices accessor (e.g. `chart.window()`) would express what every caller means; `slice`'s signature is vestigial.
- **`view.eval` always flows into `series_xy`, and nothing in between changes row count or order.** The only in-between steps are column selection (per-column `_series` in AutoPlot) and pointwise transforms (Markers' `clip(sign(...))`) — never a filter, sort, dropna, or resample. This row-preservation is what makes positional `series_xy` safe; the length half is now enforced at the boundary (`ValueError` on mismatch), while row *order* remains upheld by convention. Side note: `item=` has been dropped from Line/Bars/Area too (2026-07-19) — selection is by composition (`.struct.field(...)` / `as_expr(item=...)`), same message as Swings. A plain string is now a first-class indicator form meaning **column reference only**: `apply_indicator` resolves it as native column access (`prices[name]` — `pl.col` semantics, no derived-price aliases; typical price is `TYPPRICE()`), `is_indicator_like` accepts it, and `get_label` returns it verbatim — so `Line("close")`, `chart.plot("close")`, and `"close" @ Line()` all work. This is the IntoExpr contract at the evaluation layer, where it belongs.
- **Computed results never re-enter a frame.** Results live as Series/arrays outside prices from `eval` to `series_xy`. The one primitive needing a computed value *alongside* a prices column (Markers) is exactly why the variadic `series_xy` exists — in a frame model (`with_columns`) that need, and the variadic form, disappear.
- **`series_xy` is single-series at 12 of 13 sites.** Markers is the only variadic caller (two sources, no common frame — see above).
- **Two disjoint data idioms, split by data shape.** Frame-shaped primitives use `slice` + column reads; series-shaped primitives use `eval` + `series_xy`. No primitive mixes idioms except Swings, which dispatches between them by constructor mode.
...
- **The `Price` primitive is retired (2026-07-19).** Its accessor role is played by the bare column string (`Line("close")`), its derived prices by named indicators (`MEDPRICE`/`TYPPRICE`/`WCLPRICE`/`AVGPRICE`, talib naming). The last primitive-moonlighting-as-indicator dual role is gone.
- **`chart.prices` has left the surface (2026-07-19).** Removed; primitives read `chart.view.prices`. The original observation, now partially realized: Six runtime accesses: five are the uniform `chart.slice(chart.prices, xcol="xloc")` idiom (a no-arg windowed-prices accessor would absorb them), and one is Markers reading the close column directly — replaceable by `view.eval("close")` now that strings are indicators. Both done, the data plane shrinks to four members (`window`, `eval`, `series_xy`, `map_date`) and primitives never see the raw frame — freeing the future core to restructure it (x/width columns, projections) without any primitive depending on its shape.
