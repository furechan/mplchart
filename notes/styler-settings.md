...
Extension, shipped 2026-07-27: **key aliases** — a style-owned rename step in settings lookup (`sma` → `overlay`), which is what makes list-valued settings (color cycles) meaningful across a semantic group. See [styler-aliases.md](styler-aliases.md).
...
- **Revised 2026-07-27 — category mapping reinstated as key aliases** (design: [styler-aliases.md](styler-aliases.md); implemented the same day). Two things changed since the rejection above, both new information rather than a change of taste:
    - *The prop cycle cannot carry it.* The mplfinance survey showed mpf runs **two** cycles at once — `mavcolors` for moving averages and the stylesheet `axes.prop_cycle` for everything else — and the two are never equal (0 of 16 shipped styles). A single prop cycle cannot express "overlays use palette A, everything else palette B", so the "already shared via the pane's cycle cursors" argument does not hold for that case.
    - *The identity-keyed reserve mechanism fails a criterion nobody had applied: serializability.* Sharing one list object across roles cannot survive a JSON/TOML round-trip — the two lists become distinct objects and the grouping silently evaporates, leaving a style that looks byte-identical and behaves differently. It also breaks the shipped-style convention (`# pure data, zero imports`) by making the file's *object graph* load-bearing.
    - What survives from the rejection: a table cannot cover user expression aliases — but neither can identity sharing, so it does not discriminate between the two; both leave unclaimed indicators on the prop cycle, which is the intended "absence of opinion" behavior. The second vocabulary and resolution hop remain the real cost of aliases, accepted because aliases stay declarative string→string data.
...
| `lineplot.color` / `.alpha` / `.width` / `.linestyle` | Line | kwargs exist today (≈ mplfinance `linecolor` for `type=line`) |
| `areaplot.color` / `.alpha` | Area | kwargs exist today |
| `barplot.color` / `.alpha` / `.width` | Bars | kwargs exist today |
...
- Facet for line style: `linestyle` (matplotlib lingo, used here) vs Line's current `style` kwarg — align at retrofit.
