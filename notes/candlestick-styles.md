...
- Notably absent across all 16 mpf styles: per-direction wicks differing from the edges; any style with face/edge/wick all three mutually distinct per direction. mplfinance's full `wick.up/down` generality is unused even by its own styles — supporting the flat `wicks.color` key. *(Revised 2026-07-27: `tradingview` does set teal/red wicks, and the flat key was the only variant-less role in the grammar — replaced by `wicks.up.color`/`wicks.down.color`; neutral is both sides alike.)*
...
Naming per layer (deliberate): the renderer speaks geometry — `faceup`/`facedn`/`faceoff` (mechanism-speak is correct at the mechanism layer); the settings speak domain — `candle.off.color` (hollow-body fill, palette) and the `candle.hollow` bool flag (the kwarg's mirror — see the naming paragraph below). Resolution: hollow mode → `faceoff = candle.off.color or background`; filled → `faceoff = None`.
